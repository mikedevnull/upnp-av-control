...
nsmap = {'upnp': 'urn:schemas-upnp-org:metadata-1-0/upnp/', 'dc': 'http://purl.org/dc/elements/1.1/'}
...
def _parse_didl(xmlElement, attributes, elements):
    ...
    for attribute in attributes:
        ...
        if attribute.required and value is None:
            raise RuntimeError('required attribute %s not present in DIDL-Lite data' % attribute.name)
        data[attribute.key] = value

    for element in elements:
        value = xmlElement.findtext(element.name, namespaces=nsmap)
        if element.required and value is None:
            raise RuntimeError('required element %s not present in DIDL-Lite data' % element.name)
...
def _parse_musicitem(xmlElement):
    data = _parse_didl(xmlElement,
                       attributes=(_DidlEntry(name='id', key='id'), _DidlEntry(name='parentID', key='parentID')),
                       elements=(_DidlEntry(name='upnp:class', key='upnpclass'), _DidlEntry(name='dc:title',
                                                                                            key='title'),
                                 _DidlEntry(name='upnp:artist', key='artist',
                                            required=False), _DidlEntry(name='upnp:album', key='album', required=False),
                                 _DidlEntry(name='upnp:originalTrackNumber', key='originalTrackNumber', required=False),
                                 _DidlEntry(name='upnp:playlist', key='playlist', required=False),
                                 _DidlEntry(name='upnp:storageMedium', key='storageMedium', required=False),
                                 _DidlEntry(name='dc:contributor', key='contributor',
                                            required=False), _DidlEntry(name='dc:date', key='date', required=False)))
...
def _parse_base_object(xmlElement):
    ...
    return BaseObject(id=id, parentID=parentID, upnpclass=upnpclass, title=title)
