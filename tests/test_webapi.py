...
async def test_current_renderer(webapi_client, mocked_renderer_device):
    mocked_renderer_device.rendering_control.action(
        'GetVolume').mock.return_value = {
            'CurrentVolume': 42
        }
    ...
    response = await webapi_client.put(
        '/player/device', json={'udn': mocked_renderer_device.udn})
    assert response.status_code == 200
    response = await webapi_client.get('/player')
    assert response.status_code == 200
    playbackInfo = response.json()
...
async def test_set_volume(webapi_client, mocked_renderer_device):
    ...
    volume_action = mocked_renderer_device.rendering_control.action(
        'SetVolume')
    volume_action.mock.assert_called_once_with(InstanceID=0,
                                               Channel='Master',
                                               DesiredVolume=21)
