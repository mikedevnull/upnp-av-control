...
describe("MediaserverBrowser.vue", () => {
  it("should show a generic container listing for containers", async () => {
    ControlPoint.getObjectMetadata.mockResolvedValueOnce([
      { id: "123", title: "foo", upnpclass: "object.container.foo.bar" }
    ]);
    const wrapper = shallowMount(MediaserverBrowser);
...
describe("TrackList.vue", () => {
  it("renders correctly", () => {
    const items = [
      { title: "foo", upnpclass: "object.item.audioItem.musicTrack" },
      {
        title: "bar",
        albumArtURI: "/bar.png",
    ...
    const wrapper = mount(TrackList, {
      propsData: { items, heading: "My heading" },
...
describe("CoverImageGrid.vue", () => {
  it("renders correctly", () => {
    ...
    const wrapper = mount(CoverImageGrid, {
      propsData: { items, heading: "My heading" },
      stubs: { RouterLink: RouterLinkStub }
    });
    expect(wrapper.element).toMatchSnapshot();
...
describe("ContainerBrowserMixin", () => {
  ...
  beforeEach(async () => {
    ...
    wrapper = mount(Component, {
      propsData: { item: { id: "1234" }, udn: "456" }
    });
    await wrapper.vm.$nextTick();
  });
