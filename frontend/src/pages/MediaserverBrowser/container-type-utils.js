...
function iconForUpnpClass(upnpclass) {
  if (upnpclass.startsWith("object.container.person.musicArtist")) {
    ...
    return albumIcon;
  } else if (upnpclass.startsWith("object.item.audioItem.musicTrack")) {
    return trackIcon;
  } else {
    return folderIcon;
  }
...
function imageForItem(item) {
  if (item) {
    const uri = imageURIFromItem(item);
    if (uri) {
      return uri;
    } else {
...
function imageURIFromFirstItem(items) {
  if (items) {
    const item = items.find(x => imageURIFromItem(x) !== undefined);
    return imageURIFromItem(item);
  }
}

function guessImageForParentItem(item, children) {
  const itemImage = imageURIFromItem(item);
  if (itemImage) {
    return itemImage;
  }
  const childImage = imageURIFromFirstItem(children);
  if (childImage) {
    return childImage;
  }
  if (item) {
